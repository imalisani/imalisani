...
export default function TestCasesPage(){
  ...
  return <main className={styles.page} lang={lang}>
    <nav className={styles.nav} aria-label={lang==="es"?"Navegación de casos de prueba":"Test cases navigation"}>
      ...
      <div className={styles.navLinks}><Link href="/">← {c.back}</Link><details className={styles.navGroup} name="main-navigation"><summary className={styles.navParent}>{c.index} <span aria-hidden="true">⌄</span></summary><div className={styles.navDropdown} aria-label={lang==="es"?"Índice de subpáginas":"Subpage index"}><Link href="/como-trabajo">01 — {c.process}</Link><Link href="/casos-de-prueba" aria-current="page">02 — {c.testsNav}</Link><Link href="/repositorios">03 — {c.reposNav}</Link></div></details><details className={styles.navGroup} name="main-navigation"><summary className={styles.navParent}>{c.aboutIndex} <span aria-hidden="true">⌄</span></summary><div className={styles.navDropdown} aria-label={lang==="es"?"Índice sobre mí":"About index"}><Link href="/#work">03 — {c.metricsNav}</Link><Link href="/#about">05 — {c.experienceNav}</Link><a href="/CV-Irina-Malisani-QA-Engineer.pdf" download>CV — {c.cvNav}</a><Link href="/#contact">07 — {c.contact}</Link></div></details></div>
