...
const steps=[
  ...
  {n:"02",tone:"paper",es:{title:"Las dudas se resuelven antes",copy:"Planteo preguntas concretas en Slack al PM: “¿qué pasaría si…?”, “¿va bajo configuración o afecta a todos los clientes?”. Si el alcance lo requiere, alineamos en una reunión con Dev, PM y EM.",result:"Alcance y reglas acordadas"},en:{title:"Questions are resolved early",copy:"I ask the PM concrete questions in Slack: “what happens if…?”, “is this configuration-based or does it affect every customer?”. If scope requires it, I align with Dev, PM and EM in a meeting.",result:"Agreed scope and rules"}},
  ...
  {n:"08",tone:"violet",es:{title:"Una decisión basada en evidencia",copy:"Cruzo resultados automáticos y manuales, documento la evidencia y comunico una salida clara. La decisión no es solo pass o fail: también considera impacto, riesgo y capacidad de seguimiento.",result:"Return · Alerta · Ready for Deploy"},en:{title:"An evidence-based decision",copy:"I combine automated and manual results, document the evidence and communicate a clear outcome. The decision is not only pass or fail: it also considers impact, risk and traceability.",result:"Return · Alert · Ready for Deploy"}},
] as const;

const outcomes=[
  {key:"return",es:{title:"Return to Dev",copy:"Hay una falla explícita o un bloqueo. Dev recibe la devolución con evidencia, pasos de reproducción e impacto."},en:{title:"Return to Dev",copy:"There is an explicit failure or blocker. Dev receives the result with evidence, reproduction steps and impact."}},
  {key:"alert",es:{title:"Alerta",copy:"El riesgo no bloquea el release. Lo comunico en Slack y dejo registro en el sistema de seguimiento del equipo."},en:{title:"Alert",copy:"The risk does not block the release. I communicate it in Slack and record it in the team’s tracking system."}},
...
const copy={
  es:{back:"Portfolio",contact:"Contacto",label:"Del requerimiento al release",title:<>De historia de usuario a una <em>decisión de release.</em></>,lead:"Los agentes aceleran el análisis y la ejecución. El criterio, las preguntas y la decisión de calidad siguen siendo humanos.",chips:["Antes de In Progress","Agentes + criterio humano","Automatización + testing manual","Return · Alerta · Ready"],timeline:"El recorrido",outcomeLabel:"La salida",outcomeTitle:"Tres decisiones, una misma trazabilidad.",outcomeCopy:"Cada resultado queda comunicado y respaldado por evidencia para que el equipo sepa exactamente cómo continuar.",ctaLabel:"¿Querés ver el resultado?",ctaTitle:"Explorá los casos, repositorios y evidencia de mi portfolio.",cta:"Volver al portfolio",talk:"Hablemos"},
  en:{back:"Portfolio",contact:"Contact",label:"From requirement to release",title:<>From user story to an <em>evidence-based release decision.</em></>,lead:"Agents accelerate analysis and execution. Judgement, questions and quality decisions remain human.",chips:["Before In Progress","Agents + human judgement","Automation + manual testing","Return · Alert · Ready"],timeline:"The journey",outcomeLabel:"The outcome",outcomeTitle:"Three decisions, one traceable process.",outcomeCopy:"Every result is communicated and backed by evidence so the team knows exactly how to move forward.",ctaLabel:"Want to see the outcome?",ctaTitle:"Explore the test cases, repositories and evidence in my portfolio.",cta:"Back to portfolio",talk:"Let’s talk"},
};
...
export default function ProcessJourney(){
  ...
  return <main className={styles.page} lang={lang}>
    <nav className={styles.nav} aria-label={lang==="es"?"Navegación de Cómo trabajo":"How I work navigation"}>
      ...
      <div className={styles.navLinks}><Link href="/">← {c.back}</Link><Link href="/#contact">{c.contact}</Link></div>
    ...
    <footer className={styles.footer}><p className={styles.eyebrow}>{c.ctaLabel}</p><h2>{c.ctaTitle}</h2><div><Link href="/">{c.cta} ↗</Link><Link href="/#contact">{c.talk} ↗</Link></div></footer>
  </main>;
}
