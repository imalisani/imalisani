...
export default function ProcessJourney(){
  ...
  return <main className={styles.page} lang={lang}>
    <nav className={styles.nav} aria-label={lang==="es"?"Navegación de Cómo trabajo":"How I work navigation"}>
      ...
      <div className={styles.navLinks}><Link href="/">← {c.back}</Link><details className={styles.navGroup}><summary className={styles.navParent}>{c.index} <span aria-hidden="true">⌄</span></summary><div className={styles.navDropdown} aria-label={lang==="es"?"Índice de subpáginas":"Subpage index"}><Link href="/como-trabajo" aria-current="page">01 — {c.flowNav}</Link><Link href="/casos-de-prueba">02 — {c.casesNav}</Link><Link href="/repositorios">03 — {c.reposNav}</Link></div></details><Link href="/#contact">{c.contact}</Link></div>
