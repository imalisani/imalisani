  ...
  return <main className={styles.page} lang={lang}>
    ...
    <section className={styles.lab} aria-labelledby="lab-title">
      <div className={styles.labTop}>
        <div><p className={styles.eyebrow}>01 — {c.lab}</p><h2 id="lab-title">Refund workflow</h2></div>
      ...
      <ol className={styles.stageNav}>{stages.map((item,index)=><li key={item.id} className={index<active?styles.complete:index===active?styles.current:""}><button type="button" onClick={()=>selectStage(index)} aria-current={index===active?"step":undefined}><span>{item.n}</span><b>{item.agent}</b></button></li>)}</ol>
